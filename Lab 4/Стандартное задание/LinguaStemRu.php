...
class LinguaStemRu
{
    ...
    private $unset_preposition = false; //удалять предлоги ?
    ...
    private $PREPOSITION = '/(^|\s)(и|для|в|на|под|из|с|по)(\s|$)/u';    
    
    public function stem_string(string $words)
    {
        $word = explode(' ', $words);
        for ($i = 0; $i < count($word); $i++)
        {
                if ($this->unset_preposition === TRUE)
                {
                    $word[$i] = preg_replace($this->PREPOSITION, '', $word[$i]);
                }
        ...
        // If you need to return as single string.        
        return implode(' ', $word);

        // If you need to return as array.
        // return $word;        
    }
