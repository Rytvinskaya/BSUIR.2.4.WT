...
class Templater
{
    ...
    {	
		...
		$page_lang = Templater::get_lang($page_template);
		
		if (($page_lang != "ru") && ($page_lang != "en"))
		{
			$page_lang = 'ru';
		}		
	...
	// Returns current page language and removes {LANG="..."} template from page.
	private static function get_lang(string &$page_template)
	{
		preg_match("/({LANG=\"(.*?)\"})/u", $page_template, $matches);				
		
		$lang = isset($matches[2]) ? $matches[2] : "";
